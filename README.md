...
Interactive simulator for Jason Kelly's 9Sig strategy on synthetic TQQQ (3x QQQ), with comparisons to buy-and-hold TQQQ, QQQ, and SPY.

Live: https://9sig.networthcast.com

## Updating price data

Daily closing prices for QQQ, TQQQ, and SPY live in three TSV files at the repo root. To refresh them to the latest available date:
...
- `simulation_tqqq_qqq - qqq.tsv` (QQQ, 1999–present)
- `simulation_tqqq_qqq - tqqq.tsv` (real TQQQ, 2010–present; pre-2010 is synthesized in the browser from QQQ returns)
- `simulation_tqqq_qqq - spy.tsv` (SPY, 1993–present)
...
- **No-op on holidays**: the commit step checks `git diff --quiet -- '*.tsv'` and exits cleanly if nothing changed (market closed, no new bar).
